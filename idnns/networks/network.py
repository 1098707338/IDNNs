...
def train_network(layerSize, num_of_ephocs, learning_rate_local, batch_size, indexes, save_grads,
                  ...
                  name, covn_net):
    """Train the nework"""
    # layerSize is a list: [10, 7, 5, 4, 3]
    tf.reset_default_graph()
    data_sets = data_shuffle(data_sets_org, percent_of_train)
    ws, estimted_label, gradients, infomration, models, weights, weights_l2_norm = [[None] * len(indexes) for _ in range(7)]
    # all 4 values initialized, but never updated
    ...
    saver = tf.train.Saver(max_to_keep=0)  # not saving the model since it's so small
    ...
    with tf.Session() as sess:
        ...
        for j in range(0, num_of_ephocs):
            ...
            if j in indexes:
                if save_grads:  # this must be true in order to work
                    gradients[k] = epochs_grads
                    current_weights_mean = []
                    current_weights_l2_norm = []
                    # yuck...this is computing mean of the weights
                    # TODO: change this part...get it to compute L2 of the weights
                    for ii in range(len(current_weights)):
                        current_weights_mean.append(np.mean(np.array(current_weights[ii]), axis=0))
                        weight_layer = np.array(current_weights[ii])
                        weight_l2_penalty_norm = np.sum(weight_layer ** 2) / 2.
                        current_weights_l2_norm.append(weight_l2_penalty_norm)  # hope this works...
                    # weights_all is the weight parameters, not the gradient of weights!!
                    weights[k] = current_weights_mean
                    weights_l2_norm[k] = current_weights_l2_norm
                # Save the model
    ...
    network['weights_l2_norm'] = weights_l2_norm
    return network
